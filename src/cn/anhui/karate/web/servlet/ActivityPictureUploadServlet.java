...
@WebServlet("/ActivityPictureUploadServlet")
public class ActivityPictureUploadServlet extends HttpServlet {
            ...
            if(list!=null){
                ...
                for(FileItem item:list){
                    ...
                    if(item.isFormField()){
                        ...
                        fileList.add(bean);
                        //保存到数据库
                        service.saveActivityPicture(bean);
                    }else {
                        //封装FileBean对象
                        ...
                        //得到web应用中的某个目录的决定路径
                        String baseDir="F:/文件/newKarate/karate/web/activity/image";
                        ...
                        bean.setFilepath("activity/image"+"/"+subDir+fileName);
                    }
                }
                //把完成的文件信息转发到jsp页面显示
                request.setAttribute("fileList",fileList);
                request.getRequestDispatcher("/activityPictureUploadSuccess.jsp").forward(request,response);
