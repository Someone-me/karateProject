...
@WebServlet("/MatchUploadServlet")
public class MatchUploadServlet extends HttpServlet {
            ...
            try {
                ...
                for (FileItem item : list) {
                    if (item.isFormField()) {
                        ...
                        //得到web应用中的某个目录的决定路径
                        String baseDir = "F:/文件/newKarate/karate/web/notification/images";
                        ...
                        bean.setFilepath("notification/images" + "/" + subDir + name);
                    }

                }
                newsList.add(bean);
                //保存到数据库
                service.saveNotification(bean);
                //把完成的文件信息转发到jsp页面显示
                request.setAttribute("newsList", newsList);
                request.getRequestDispatcher("/matchUploadSuccess.jsp").forward(request, response);
