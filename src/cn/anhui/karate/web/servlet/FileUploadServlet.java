...
public class FileUploadServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            //限制文件大小
            upload.setFileSizeMax(1*1024*1024*1024);//每个文件
            upload.setSizeMax(5*1024*1024*1024);//总文件
            //解析文件
            ...
            if(list!=null){
                ...
                for(FileItem item:list){
                    ...
                    if(item.isFormField()){
                        ...
                        //得到web应用中的某个目录的决定路径
                        String baseDir="F:/文件/newKarate/karate/web/file";
                        ...
                        bean.setFilepath("/file"+"/"+subDir+fileName);
                    }
                }
                //把完成的文件信息转发到jsp页面显示
                request.setAttribute("fileList",fileList);
                request.getRequestDispatcher("/fileSuccess.jsp").forward(request,response);

            }


        }catch (FileUploadBase.FileSizeLimitExceededException e){
            request.setAttribute("msg", "单个文件超过设定的大小：1GB");
            request.getRequestDispatcher("/fileUpload.jsp").forward(request, response);
        }catch (FileUploadBase.SizeLimitExceededException e){
            request.setAttribute("msg", "超过总文件大小：5GB");
            request.getRequestDispatcher("/fileUpload.jsp").forward(request, response);
        }
