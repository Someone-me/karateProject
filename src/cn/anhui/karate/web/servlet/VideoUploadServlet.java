...
public class VideoUploadServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ArrayList<VideoBean> newsList = new ArrayList<VideoBean>();
        String fieldName = "";
        //判断是表单是否为多部分组成
        if (isMultipart == true) {
                ...
                for (FileItem item : list) {
                    if (item.isFormField()) {
                        ...
                        if (fieldName.equals("url")) {
                            bean.setUrl(item.getString("utf-8"));
                        }
                        ...
                        //得到web应用中的某个目录的决定路径
                        String baseDir = "F:/文件/newKarate/karate/web/video/image";
                        ...
                        item.delete();
                        bean.setName(name);
                        bean.setFilepath("video/image" + "/" + subDir + name);
                    }

                }
                newsList.add(bean);
                //保存到数据库
                service.saveVideo(bean);
                //把完成的文件信息转发到jsp页面显示
                request.setAttribute("newsList", newsList);
                request.getRequestDispatcher("/videoUploadSuccess.jsp").forward(request, response);
